                ...
                if part.get("filename"):
                    ...
                    if att_id:
                        ...
                        if data:
                            ...
                            file_path = _save_file(target_dir, part["filename"], file_data)
...
def fetch_mails_in_date(allowed_subjects):
    ...
    for thread_id, msgs in threads.items():
        ...
        for msg in msgs_sorted:
            attachments_info = []
            if msg.attachments:
                safe_subject = re.sub(r'[\\/*?:"<>|]', "_", msgs_sorted[0].subject or "no_subject")
                thread_dir = os.path.join("outputs", f"{thread_id}_{safe_subject}")
                attachments_info = download_attachments_parent(gmail.service, "me", msg.id, thread_dir)
        ...
        if all_attachments:
            summary_filename = os.path.join(thread_dir, f"{thread_id}_attachment_summary.md")
            with open(summary_filename, "w", encoding="utf-8") as f:
                ...
                for i, att_info in enumerate(all_attachments, 1):
                    ...
                    if att_info.get('attachment_context'):
                        ...
                        f.write("**Nội dung:** Không thể trích xuất nội dung từ file này.\n\n")
                    f.write("\n\n---\n\n")
